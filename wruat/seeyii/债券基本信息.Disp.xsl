...
$().ready(function(){
	...
	// 起息日
	$("#qxri").text(formatDateTime($("#qxri").text()));
	// 到期兑付日
	$("#dqdfri").text(formatDateTime($("#dqdfri").text()));

	// 债券类别
	var zqlb = $("#_x503a__x5238__x7c7b__x522b_").text();
	if (zqlb != ""){
		// Table: 债券常量表
		parameters = "/_api/web/lists/getbytitle('债券常量表')/items?$filter=(_x5e38__x91cf__x5206__x7c7b__x7f eq 1030) and (_x6574__x578b__x503c_ eq "+zqlb+")";
		getItemsFromSeeyiiView(parameters, function(v){
			// 债券类别
			$("#foreign_zqlb").text(formatRestResult(v[0].OData__x5e38__x91cf__x63cf__x8ff0_));
		});
	} else{
		$("#foreign_zqlb").text("--");
	}
	...
	// Table: 债券要素新表
	parameters = "/_api/web/lists/getbytitle('债券要素新表')/items?$filter=_x503a__x5238_id eq "+zqid;
	getItemsFromSeeyiiView(parameters, function(v){
		// 主体评级
		$("#foreign_ztpj").text(formatRestResult(v[0].OData__x503a__x9879__x4fe1__x7528__x7e));
		// 发行日
		$("#foreign_fxri").text(formatDateTime(v[0].OData__x53d1__x884c__x65e5_));
		// 上市日
		$("#foreign_ssri").text(formatDateTime(v[0].OData__x4e0a__x5e02__x65e5__x0028__x4e));
	});
	...
	<tr class="page-variable">
		<td>
			<span id="_x503a__x5238_id"><xsl:value-of select="@_x503a__x5238_id"/></span>
			<span id="_x503a__x5238__x7c7b__x522b_"><xsl:value-of select="@_x503a__x5238__x7c7b__x522b_"/></span>
		</td>
	</tr>
	<tr>
		<td class="under-line-border row-title">
			<span class="page-title"><xsl:value-of select="@_x503a__x5238__x7b80__x79f0_"/>(<xsl:value-of select="@_x503a__x5238__x4ee3__x7801_"/>)</span>
		</td>
	</tr>
	<tr>
		<td>
			<table border="0" cellspacing="0" width="100%" style="border-collapse:collapse" id="MainTable">
				...
				<tr>
					...
					<td class="ms-formbody under-line-border" width="35%" id="foreign_ztpj"></td>
				...
				<tr>
					...
					<td class="ms-formbody" colspan="3"><xsl:value-of select="@_x62c5__x4fdd__x4eba_"/></td>
				</tr>
				<tr>
					<td class="ms-formlabel">发行日：</td>
					<td class="ms-formbody" id="foreign_fxri"></td>
					<td class="ms-formlabel">上市日：</td>
					<td class="ms-formbody" id="foreign_ssri"></td>
				</tr>
				<tr>
					<td class="ms-formlabel under-line-border">起息日：</td>
					<td class="ms-formbody under-line-border" id="qxri"><xsl:value-of select="@_x8d77__x606f__x65e5_"/></td>
					<td class="ms-formlabel under-line-border">到期兑付日：</td>
					<td class="ms-formbody under-line-border" id="dqdfri"><xsl:value-of select="@_x5230__x671f__x65e5_"/></td>
				</tr>
				<tr>
